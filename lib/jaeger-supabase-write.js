...
async function callWriteRpc(fn, params, requestId) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      lastError = error;
      // Financial/catalog validation is deterministic. Retrying a rejected
      // 4xx request only adds latency and load while returning the same result.
      if (Number(error && error.status) >= 400 && Number(error && error.status) < 500) break;
    }
  }
  throw lastError;
}
