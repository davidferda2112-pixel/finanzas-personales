...
async function restRequest(path, options) {
  ...
  if (!response.ok) {
    // PostgREST includes the database error in its response body.  Preserve a
    // bounded, server-side-only diagnostic so a rejected financial write can
    // be fixed from production evidence instead of being reported as a vague
    // connection failure.
    let diagnostic = '';
    let parsedBody = null;
    try {
      parsedBody = text ? JSON.parse(text) : null;
      diagnostic = [parsedBody && parsedBody.code, parsedBody && parsedBody.message,
        parsedBody && parsedBody.details, parsedBody && parsedBody.hint]
        .filter(Boolean)
        .join(' | ');
    } catch (_) {
      diagnostic = text;
    }
    diagnostic = String(diagnostic || '').replace(/[\r\n]+/g, ' ').slice(0, 800);
    const error = new Error(`Supabase respondio HTTP ${response.status}${diagnostic ? `: ${diagnostic}` : ''}`);
    error.name = 'SupabaseHttpError';
    error.status = response.status;
    error.supabaseCode = parsedBody && parsedBody.code ? String(parsedBody.code) : '';
    error.publicMessage = parsedBody && parsedBody.message
      ? String(parsedBody.message).replace(/[\r\n]+/g, ' ').slice(0, 300)
      : '';
    throw error;
  }
