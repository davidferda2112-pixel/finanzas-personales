...
async function restRequest(path, options) {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      diagnostic = [body && body.code, body && body.message, body && body.details, body && body.hint]
        .filter(Boolean)
        .join(' | ');
    } catch (_) {
      diagnostic = text;
    ...
    throw new Error(`Supabase respondio HTTP ${response.status}${diagnostic ? `: ${diagnostic}` : ''}`);
  }
