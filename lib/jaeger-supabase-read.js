...
const SUPPORTED_READS = new Set([
  ...
  'getCatalogoFinanciero', 'getDiferidosTdc', 'getNotificaciones',
  'getConfiguracion'
]);

const EXTENDED_SOURCE_READS = new Set([
  'getCatalogoFinanciero', 'getDiferidosTdc', 'getNotificaciones'
]);

const CARD_SOURCE_READS = new Set([
  'getTarjetasState', 'parseTarjetas', 'getMovimientosTarjeta'
]);

async function getNativeReadStatus() {
  const result = await restRequest('rpc/jaeger_native_read_status', {
    method: 'POST', body: '{}'
  });
...
async function markNativeReadsStale(reason) {
  const result = await restRequest('rpc/jaeger_mark_native_reads_stale', {
    method: 'POST', body: JSON.stringify({ p_reason: String(reason || 'legacy_write').slice(0, 120) })
  });
  if (!result.ok) throw new Error(result.error || 'No se pudo invalidar la lectura nativa');
  return { ok: true, state: result.text ? JSON.parse(result.text) : {} };
}
...
function buildJapan(source) {
  ...
  const totalManual = actualRow ? number(actualRow.actual) : 0;
  const asignaciones = (source.allocations || []).map((item) => ({
    balanceId: String(item.balance_id || ''), nombre: String(item.asset_name || ''),
    grupo: String(item.group_name || ''), asignado: number(item.allocated_amount),
    disponible: number(item.asset_value), efectivo: number(item.effective_amount),
    activo: item.asset_active !== false
  }));
  const totalVinculado = money(asignaciones.reduce((sum, item) => sum + item.efectivo, 0));
  const totalAsignado = money(asignaciones.reduce((sum, item) => sum + item.asignado, 0));
  const totalActual = money(totalManual + totalVinculado);
  return { ok: true, items, tramites, totalPresupuesto, totalManual, totalVinculado,
    totalAsignado, totalActual, asignaciones,
    faltante: money(Math.max(0, totalPresupuesto - totalActual)),
    porcentaje: totalPresupuesto ? totalActual / totalPresupuesto * 100 : 0 };
}
...
function buildCards(source) {
  ...
  const configured = (source.cards && source.cards.length ? source.cards : [
    { card_code: 'VISA', display_name: 'Visa Personal', issuer: 'Banco Internacional',
      last_four: '4894', network: 'visa', card_style: 'visa-card', active: true, sort_order: 1 },
    { card_code: 'MC', display_name: 'Mastercard Gold GC', issuer: 'Banco Internacional',
      last_four: '9593', network: 'mastercard', card_style: 'mc-card', active: true, sort_order: 2 }
  ]).filter((card) => card.active !== false)
    .sort((a, b) => number(a.sort_order) - number(b.sort_order) || String(a.card_code).localeCompare(String(b.card_code)));
  return configured.map((card) => {
    const code = String(card.card_code || '').trim();
    const network = String(card.network || 'other').toLowerCase();
    return {
      id: code,
      nombre: String(card.display_name || code),
      emisor: String(card.issuer || ''),
      numero: `**** **** **** ${String(card.last_four || '0000')}`,
      ultimos4: String(card.last_four || '0000'),
      clase: String(card.card_style || 'generic-card'),
      logo: network === 'mastercard' ? 'mc' : network,
      red: network,
      historial2026: makeHistory(code, 2026),
      meses2026: SHORT_MONTHS.map((month) => `${month} 26`),
      historial2025: makeHistory(code, 2025),
      meses2025: SHORT_MONTHS.map((month) => `${month} 25`),
      diferidos: makeInstallments(code)
    };
  });
}

function buildConfiguration(source) {
  const allocations = source.allocations || [];
  const cards = (source.cards || []).map((card) => ({
    codigo: String(card.card_code || ''), nombre: String(card.display_name || ''),
    emisor: String(card.issuer || ''), ultimos4: String(card.last_four || ''),
    red: String(card.network || 'other'), estilo: String(card.card_style || 'generic-card'),
    activo: card.active !== false, orden: number(card.sort_order), version: number(card.row_version)
  }));
  const assets = (source.assets || []).map((asset) => {
    const allocation = allocations.find((item) => item.goal_key === 'japan' && item.balance_id === asset.balance_id);
    return {
      codigo: String(asset.balance_id || ''), nombre: String(asset.asset_name || ''),
      grupo: String(asset.group_name || ''), valor: number(asset.asset_value),
      activo: asset.asset_active !== false, asignadoJapon: number(allocation && allocation.allocated_amount),
      efectivoJapon: number(allocation && allocation.effective_amount)
    };
  });
  return {
    ok: true,
    fuente: String(source.activeSource || 'supabase'),
    timezone: String(source.timezone || 'America/Guayaquil'),
    tarjetas: cards,
    activos: assets,
    totalAsignadoJapon: money(assets.reduce((sum, asset) => sum + asset.asignadoJapon, 0)),
    totalEfectivoJapon: money(assets.reduce((sum, asset) => sum + asset.efectivoJapon, 0))
  };
}
...
function buildCatalog(source) {
  const catalog = (source.catalog || []).filter((item) => item.state === 'activo');
  const itemForApp = (item) => ({
    id: String(item.legacy_id || ''), tipo: String(item.kind || ''), nombre: String(item.name || ''),
    estado: String(item.state || 'activo'), balanceTipo: String(item.balance_type || ''),
    balanceId: String(item.balance_id || ''), balanceNombre: String(item.balance_name || ''),
    grupo: String(item.group_name || ''), orden: number(item.sort_order), nota: String(item.note || '')
  });
  const savings = catalog.filter((item) => item.kind === 'ahorro').map(itemForApp);
  const debts = catalog.filter((item) => item.kind === 'deuda').map(itemForApp);
  const balance = buildBalance(source);
  const destination = (items) => (items || []).filter((item) => !item.esGrupo).map((item) => ({
    codigo: item.codigo, id: item.codigo, nombre: item.nombre, grupo: item.grupo,
    valor: item.valor, tipo: item.tipo
  }));
  const groups = (source.groups || []).filter((group) => group.active !== false)
    .sort((a, b) => number(a.sort_order) - number(b.sort_order));
  return { ok: true, data: {
    subcats: {
      ahorro: savings.map((item) => item.nombre), deuda: debts.map((item) => item.nombre),
      ingreso: ['Devolución de ahorro', 'Préstamos recibidos']
    },
    items: { ahorro: savings, deuda: debts },
    activos: destination(balance.activos), pasivos: destination(balance.pasivos),
    grupos: {
      Activo: groups.filter((group) => group.balance_type === 'Activo').map((group) => group.name),
      Pasivo: groups.filter((group) => group.balance_type === 'Pasivo').map((group) => group.name)
    }
  } };
}

function buildDeferreds(source, card, month) {
  const targetOrder = monthOrder(month);
  const data = (source.installments || []).filter((item) => item.state === 'activo' &&
    (!card || item.card_code === card)).map((item) => {
    const baseOrder = monthOrder(item.base_month);
    let collected = number(item.installments_at_base_month) + (targetOrder - baseOrder);
    if (!Number.isFinite(collected) || collected < 0) collected = 0;
    const balance = money(Math.max(0, number(item.initial_balance) - number(item.installment_amount) * collected));
    return {
      id: String(item.legacy_id || ''), row: number(item.source_row_number), tarjeta: item.card_code,
      nombre: item.name, inicial: number(item.initial_balance), cuota: number(item.installment_amount),
      cuotasAlMesBase: number(item.installments_at_base_month), mesBase: shortMonth(item.base_month),
      estado: item.state || 'activo', mesLiquidacion: item.liquidation_month || '',
      balanceId: item.balance_id || '', saldoActual: balance
    };
  });
  return { ok: true, data };
}

function buildNotifications(source) {
  return { ok: true, data: (source.notifications || []).map((item) => ({
    id: String(item.notification_id || ''), texto: String(item.message || item.title || ''),
    fecha: dateText(item.business_date || item.occurred_at), leida: Boolean(item.read_at)
  })) };
}
...
function transformSource(fn, args, source) {
  ...
  if (fn === 'getCatalogoFinanciero') return buildCatalog(source);
  if (fn === 'getDiferidosTdc') return buildDeferreds(source, String(args[0] || ''), args[1]);
  if (fn === 'getNotificaciones') return buildNotifications(source);
  if (fn === 'getConfiguracion') return buildConfiguration(source);
...
async function nativeSource(fn, args) {
  let rpc = 'rpc/jaeger_native_read_source';
  if (EXTENDED_SOURCE_READS.has(fn)) rpc = 'rpc/jaeger_native_read_source_extended';
  else if (CARD_SOURCE_READS.has(fn)) rpc = 'rpc/jaeger_card_read_source';
  else if (fn === 'getViajeJapon') rpc = 'rpc/jaeger_goal_read_source';
  else if (fn === 'getConfiguracion') rpc = 'rpc/jaeger_configuration_source';
  const result = await restRequest(rpc, {
...
async function buildInitialState(args) {
  ...
  const monthsSource = await nativeSource('getMesesDisponibles', []);
  const months = transformSource('getMesesDisponibles', [], monthsSource).data || [];
  const current = calendarMonth();
  let homeMonth = normalizeMonth(options.homeMes || current);
  if (!months.includes(homeMonth)) homeMonth = months.includes(current) ? current : (months[months.length - 1] || homeMonth);
  let historyMonth = normalizeMonth(options.histMes || homeMonth);
  if (!months.includes(historyMonth)) historyMonth = homeMonth;
  let cardMonth = normalizeMonth(options.cardMes || homeMonth);
  if (!months.includes(cardMonth)) cardMonth = homeMonth;
  const cardIndex = Number.parseInt(options.cardIdx, 10) || 0;
  const cardYear = Number.parseInt(options.cardYear, 10) || new Date().getFullYear();
  const requests = [
    nativeSource('getMesData', [homeMonth]),
    historyMonth === homeMonth ? null : nativeSource('getMesData', [historyMonth]),
    nativeSource('getMovimientosMes', [historyMonth]),
    nativeSource('getTarjetasState', [{ mes: cardMonth, idx: cardIndex, anio: cardYear }]),
    nativeSource('getPinturasMes', [homeMonth])
  ];
  if (options.includeSecondary) {
    requests.push(nativeSource('getFlujoCaja', []), nativeSource('getBalanceGeneral', []), nativeSource('getViajeJapon', []));
  }
  const sources = await Promise.all(requests);
  const home = transformSource('getMesData', [homeMonth], sources[0]);
  const output = {
    ok: true, generatedAt: new Date().toISOString(), meses: months, mesActual: homeMonth, histMes: historyMonth,
    home, hist: historyMonth === homeMonth ? home : transformSource('getMesData', [historyMonth], sources[1]),
    movimientos: transformSource('getMovimientosMes', [historyMonth], sources[2]),
    tarjetas: transformSource('getTarjetasState', [{ mes: cardMonth, idx: cardIndex, anio: cardYear }], sources[3]),
    pinturas: transformSource('getPinturasMes', [homeMonth], sources[4])
  };
  if (options.includeSecondary) {
    output.flujo = transformSource('getFlujoCaja', [], sources[5]);
    output.balance = transformSource('getBalanceGeneral', [], sources[6]);
    output.japon = transformSource('getViajeJapon', [], sources[7]);
  }
  return output;
}
...
function calendarMonth() {
  const parts = new Intl.DateTimeFormat('es-EC', {
    timeZone: 'America/Guayaquil', month: 'long', year: '2-digit'
  }).format(new Date()).replace(' de ', ' ');
  return normalizeMonth(parts.charAt(0).toUpperCase() + parts.slice(1));
}

async function buildBootState(args) {
  const options = (args && args[0]) || {};
  const monthsSource = await nativeSource('getMesesDisponibles', []);
  const months = transformSource('getMesesDisponibles', [], monthsSource).data || [];
  const current = calendarMonth();
  ...
  const requests = [nativeSource('getMesData', [homeMonth])];
  if (options.includeHist && historyMonth !== homeMonth) requests.push(nativeSource('getMesData', [historyMonth]));
  if (options.includeMovimientos) requests.push(nativeSource('getMovimientosMes', [historyMonth]));
  if (options.includePinturas) requests.push(nativeSource('getPinturasMes', [homeMonth]));
  const sources = await Promise.all(requests);
  let index = 0;
  const home = transformSource('getMesData', [homeMonth], sources[index++]);
  const output = { ok: true, generatedAt: new Date().toISOString(), meses: months,
    mesActual: homeMonth, histMes: historyMonth, home };
  if (options.includeHist) {
    output.hist = historyMonth === homeMonth ? home : transformSource('getMesData', [historyMonth], sources[index++]);
  }
  if (options.includeMovimientos) {
    output.movimientos = transformSource('getMovimientosMes', [historyMonth], sources[index++]);
  }
  if (options.includePinturas) {
    output.pinturas = transformSource('getPinturasMes', [homeMonth], sources[index++]);
  }
  return output;
}

async function nativeRead(fn, args, options) {
  if (!SUPPORTED_READS.has(fn)) return { ok: false, supported: false };
  const started = Date.now();
  const requireFresh = options && options.requireFresh;
  const statusPromise = requireFresh ? getNativeReadStatus() : Promise.resolve(null);
  const dataPromise = fn === 'getBootState'
    ? buildBootState(args || [])
    : fn === 'getInitialState'
      ? buildInitialState(args || [])
      : nativeSource(fn, args || []).then((source) => transformSource(fn, args || [], source || {}));
  const [status, data] = await Promise.all([statusPromise, dataPromise]);
  if (requireFresh && (!status.enabled || !status.fresh)) {
    return { ok: false, supported: true, stale: true, status: status.state, durationMs: Date.now() - started };
  }
  return { ok: true, supported: true, data, status: status && status.state, durationMs: Date.now() - started };
}
