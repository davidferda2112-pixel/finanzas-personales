-- Imported card history is the immutable baseline. Operational installments
-- must still adjust that baseline: new Supabase installments add to it and an
-- imported installment liquidated in a month stops contributing from the next
-- month onward.
      ...
      when abs(b.base_deferred) > 0.004 then
        round(greatest(
          ...
          b.base_deferred
            ...
            + coalesce(iv.operational_deferred, 0)
        ), 2)
  ...
  left join lateral (
    select
      sum(x.remaining_amount) filter (
        where not x.stops_after_liquidation
      ) as full_deferred,
      sum(x.remaining_amount) filter (
        where x.source_kind = 'supabase'
          and not x.stops_after_liquidation
      ) as operational_deferred,
      sum(x.remaining_amount) filter (
        where x.source_kind is distinct from 'supabase'
          and x.stops_after_liquidation
...
revoke all on table jaeger.card_history_effective
  from public, anon, authenticated;
grant select on table jaeger.card_history_effective
  to service_role;

comment on view jaeger.card_history_effective is
  'Historial efectivo: el mes de liquidación conserva su saldo diferido y los meses posteriores excluyen el diferido liquidado.';
